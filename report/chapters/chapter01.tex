...
A  \textbf{\acrfull{mpg}} is a perfect information, zero-sum, turn based game played indefinitively on a Mean Payoff Graph as follow:
\begin{itemize}
...
\begin{figure}[ht]
	...
	\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2cm,
		...
		(4) edge node {3} (2);
	\end{tikzpicture} 
	\caption{An example of a \acrshort{mpg}}
\end{figure}
\subsection{Well Foundness}
It is not very clear from the definition that the game is well founded. \newline
In fact, there are choices for which the mean payoff does not converge. That is the sequence $\displaystyle\left(\frac{1}{n}\sum_{k=0}^{n-1} w_k \right)_{n\in\mathbb{N}^*}$ does not converge.  
...
An example of an execution that gives a rise to such payoffs is the following \acrshort{mpg} instance:
\begin{figure}[H]
	\centering
...
In this report, we will use the term ``positional strategies'' as an alias for memoryless deterministic strategies, which is conforming to the established litterature of \acrshortpl{mpg}.
...
\begin{theorem}
	...
	For all \acrlongpl{mpg}, each player has an optimal positional strategy.
\end{theorem}
...
In this report, we will use both the terms ``fractional strategies'' and ``behavioural strategies" interchangeably as an alias for memoryless probabilistic strategies, which is also conforming to the established litterature of \acrshortpl{mpg}.
...
\section*{Conclusion}
In this chapter, we formally defined \acrshortpl{mpg}, the strategies, and the mean payoff itself. We showed that while it can be problematic to have a mean payoff in the general sense, This will not be an issue in our scope.
